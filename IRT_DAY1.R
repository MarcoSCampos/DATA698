...
# random sample to compute IRT 10000 samples
#set.seed(123)
...
write.csv(sample_enem, file = "sample_enem_day1.csv",row.names=TRUE)
rm(sample_enem)
rm(to_remove)
...
# Run IRT function Day 1
nrm_day1<-mirt(df_day1,1,'nominal')

# See IRT parameters Day 1
ipar.nrm_day1<-coef(nrm_day1,simplify=T,IRTpars=TRUE)$item
head(ipar.nrm_day1) # IRT parameters day1

# Theta parameter Day 1
theta_Day1.ML<-fscores(nrm_day1,method='ML')
head(theta_Day1.ML)
...
m4<-function(std1,std2){
  th1_Day1<-theta_Day1.ML[std1,1]
  th2_Day1<-theta_Day1.ML[std2,1]
  P1_Day1<-irtprob(item.param=ipar.nrm_day1,th=th1_Day1)
  P2_Day1<-irtprob(item.param=ipar.nrm_day1,th=th2_Day1)
  P_Day1<-P1_Day1[,5]*P2_Day1[,5]
  Q_Day1<-rowSums(P1_Day1[,1:4]*P2_Day1[,1:4])
  m_Day1 <- sum((df_day1[std1,]==df_day1[std2,] & df_day1[std1,]==5)*1)
  n_Day1 <- sum((df_day1[std1,]==df_day1[std2,] & df_day1[std1,]!=5)*1)
  M4_Day1 <- gtd(P=P_Day1,Q=Q_Day1,m=m_Day1,n=n_Day1)
  return (M4_Day1[[2]])
}
...
m4plot<-function(std1,std2){
  ...
  plot_obj<-contourplot(TailProbability ~ CorrectMatch + IncorrectMatch,
                        ...
                        main=paste("Contour Plot for",std1,std2,"Day 1", sep=" "),
                        ...
                        scales=list(
                          y=list(at=seq(0,85,4)),
                          x=list(at=seq(0,85,4)))
  )
print(plot_obj) 
}
